...
def test_spark_udf_spark_connect(spark):
    ...
    with qcflow.start_run():
        info = qcflow.sklearn.log_model(model, "model")
    sdf = spark.createDataFrame(pd.DataFrame(X, columns=list("abcd")))
    udf = qcflow.pyfunc.spark_udf(spark, info.model_uri, env_manager="local")
...
def test_spark_udf_spark_connect_with_model_logging(spark, tmp_path):
    ...
    qcflow.set_tracking_uri(tmp_path.joinpath("mlruns").as_uri())
    qcflow.set_experiment("test")
    with qcflow.start_run():
        signature = qcflow.models.infer_signature(X, y)
        model_info = qcflow.sklearn.log_model(model, "model", signature=signature)

    udf = qcflow.pyfunc.spark_udf(spark, model_info.model_uri, env_manager="local")
    X_test = X.head(5)
