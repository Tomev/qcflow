...
def test_dataset_construction_validates_features_and_targets():
    ...
    with pytest.raises(
        ...
        qcflow.data.from_tensorflow(features=tf_tensor, targets=tf_dataset)
    ...
    with pytest.raises(
        ...
        qcflow.data.from_tensorflow(features=tf_dataset, targets=tf_tensor)
...
def test_from_tensorflow_dataset_constructs_expected_dataset():
    x = np.random.sample((100, 2))
    tf_dataset = tf.data.Dataset.from_tensors(x)
    qcflow_ds = qcflow.data.from_tensorflow(tf_dataset, source="my_source")
    assert isinstance(qcflow_ds, TensorFlowDataset)
    assert qcflow_ds.data == tf_dataset
...
def test_from_tensorflow_dataset_with_targets_constructs_expected_dataset():
    ...
    qcflow_ds = qcflow.data.from_tensorflow(tf_dataset_x, source="my_source", targets=tf_dataset_y)
    assert isinstance(qcflow_ds, TensorFlowDataset)
    assert qcflow_ds.data == tf_dataset_x
    assert qcflow_ds.targets == tf_dataset_y
    assert qcflow_ds.schema == TensorDatasetSchema(
...
def test_from_tensorflow_tensor_constructs_expected_dataset():
    x = np.random.sample((100, 2))
    tf_tensor = tf.convert_to_tensor(x)
    qcflow_ds = qcflow.data.from_tensorflow(tf_tensor, source="my_source")
    assert isinstance(qcflow_ds, TensorFlowDataset)
    # compare if two tensors are equal using tensorflow utils
    assert tf.reduce_all(tf.math.equal(qcflow_ds.data, tf_tensor))
    assert qcflow_ds.schema == TensorDatasetSchema(features=_infer_schema(tf_tensor.numpy()))
    assert qcflow_ds.profile == {
        "features_cardinality": tf.size(tf_tensor).numpy(),
    }
...
def test_from_tensorflow_tensor_with_targets_constructs_expected_dataset():
    ...
    qcflow_ds = qcflow.data.from_tensorflow(tf_tensor_x, source="my_source", targets=tf_tensor_y)
    assert isinstance(qcflow_ds, TensorFlowDataset)
    assert tf.reduce_all(tf.math.equal(qcflow_ds.data, tf_tensor_x))
    assert tf.reduce_all(tf.math.equal(qcflow_ds.targets, tf_tensor_y))
    assert qcflow_ds.schema == TensorDatasetSchema(
...
def test_from_tensorflow_no_source_specified():
    x = np.random.sample((100, 2))
    tf_dataset = tf.data.Dataset.from_tensors(x)
    qcflow_ds = qcflow.data.from_tensorflow(tf_dataset)
...
def test_digest_computation_succeeds_with_none_element_in_numpy_iterator():
    ...
    qcflow_ds = qcflow.data.from_tensorflow(tf_dataset)
    assert qcflow_ds.digest == "bc8ef018"
