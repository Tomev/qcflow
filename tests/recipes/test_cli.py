...
def clean_up_recipe():
    try:
        ...
        CliRunner().invoke(recipes_cli.clean, env={QCFLOW_RECIPES_PROFILE.name: "local"})
...
def test_recipes_cli_step_works(step):
    for command in [recipes_cli.clean, recipes_cli.inspect, recipes_cli.run]:
        assert (
            CliRunner()
            .invoke(command, args=f"--step {step}", env={QCFLOW_RECIPES_PROFILE.name: "local"})
            .exit_code
            == 0
        )
...
def test_recipes_cli_flow_completes_successfully():
    for command in [recipes_cli.clean, recipes_cli.inspect, recipes_cli.run]:
        assert (
            CliRunner().invoke(command, env={QCFLOW_RECIPES_PROFILE.name: "local"}).exit_code == 0
        )
...
def test_recipes_cli_fails_without_profile():
    for command in [recipes_cli.clean, recipes_cli.inspect, recipes_cli.run]:
        assert CliRunner().invoke(command, env={QCFLOW_RECIPES_PROFILE.name: ""}).exit_code != 0
...
def test_recipes_cli_works_with_non_default_profile():
    result = CliRunner().invoke(recipes_cli.clean, env={QCFLOW_RECIPES_PROFILE.name: "databricks"})
