...
def test_log_figure_matplotlib(subdir):
    ...
    with qcflow.start_run():
        qcflow.log_figure(fig, artifact_file)
        plt.close(fig)
...
def test_log_figure_save_kwargs():
    ...
    with qcflow.start_run():
        ...
        qcflow.log_figure(fig, name, save_kwargs={"div_id": div_id})
        artifact_uri = qcflow.get_artifact_uri(name)
...
def test_log_figure_raises_error_for_unsupported_figure_object_type():
    with qcflow.start_run(), pytest.raises(TypeError, match="Unsupported figure object type"):
        qcflow.log_figure("not_figure", "figure.png")
