...
from qcflow.utils.model_utils import env_var_tracker
...
def test_get_flavor_configuration_throws_exception_when_requested_flavor_is_missing(
    ...
    qcflow.sklearn.save_model(sk_model=sklearn_knn_model, path=model_path)

    # The saved model contains the "sklearn" flavor, so this call should succeed
    sklearn_flavor_config = qcflow_model_utils._get_flavor_configuration(
...
def test_get_flavor_configuration_with_present_flavor_returns_expected_configuration(
    ...
    assert sklearn_flavor_config == model_config.flavors[qcflow.sklearn.FLAVOR_NAME]
...
def test_add_code_to_system_path(sklearn_knn_model, model_path):
    ...
    with TempDir(chdr=True):
        ...
        qcflow_model_utils._add_code_from_conf_to_system_path(model_path, sklearn_flavor_config)
    ...
    # happens if QCFlow is misconfiguring the system path.
