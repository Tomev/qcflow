...
from qcflow.types.schema import ColSpec, ParamSchema, ParamSpec, Schema, TensorSpec
...
def test_log_model():
    ...
    loaded_model = qcflow.openai.load_model(model_info.model_uri)
    assert loaded_model["model"] == "gpt-4o-mini"
    assert loaded_model["task"] == "chat.completions"
    assert loaded_model["temperature"] == 0.9
    assert loaded_model["messages"] == [{"role": "system", "content": "You are an QCFlow expert."}]
...
def test_chat_multiple_variables(tmp_path):
    ...
    model = qcflow.models.Model.load(tmp_path)
...
def test_task_argument_accepts_class(tmp_path):
    qcflow.openai.save_model(model="gpt-4o-mini", task=chat_completions(), path=tmp_path)
    loaded_model = qcflow.openai.load_model(tmp_path)
    assert loaded_model["task"] == "chat.completions"
...
def test_model_argument_accepts_retrieved_model(tmp_path):
    ...
    qcflow.openai.save_model(model=model, task=chat_completions(), path=tmp_path)
    loaded_model = qcflow.openai.load_model(tmp_path)
    assert loaded_model["model"] == "gpt-4o-mini"
...
def test_save_model_with_secret_scope(tmp_path, monkeypatch):
    scope = "test"
    monkeypatch.setenv("QCFLOW_OPENAI_SECRET_SCOPE", scope)
    with (
        mock.patch("qcflow.openai.is_in_databricks_runtime", return_value=True),
        mock.patch("qcflow.openai.check_databricks_secret_scope_access"),
    ):
        with pytest.warns(FutureWarning, match="QCFLOW_OPENAI_SECRET_SCOPE.+deprecated"):
            qcflow.openai.save_model(model="gpt-4o-mini", task="chat.completions", path=tmp_path)
...
def test_spark_udf_chat(tmp_path, spark):
    ...
    udf = qcflow.pyfunc.spark_udf(spark, tmp_path, result_type="string")
...
class ChatCompletionModel(qcflow.pyfunc.PythonModel):
...
def test_embeddings_pyfunc_server_and_score():
    ...
    expected = qcflow.pyfunc.load_model(model_info.model_uri).predict(df)
...
def test_spark_udf_embeddings(tmp_path, spark):
    ...
    udf = qcflow.pyfunc.spark_udf(spark, tmp_path, result_type="array<double>")
    df = spark.createDataFrame(
        [
            ("a",),
            ("b",),
...
def test_inference_params(tmp_path):
    ...
    model_info = qcflow.models.Model.load(tmp_path)
    assert (
