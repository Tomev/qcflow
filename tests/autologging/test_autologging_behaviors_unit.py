...
@pytest.fixture
def logger():
    return logging.getLogger(qcflow.__name__)
...
def autolog_function(patch_destination, logger):
    ...
    def patch_impl(original):
        ...
        warnings.warn_explicit(
            "preamble QCFlow warning", category=Warning, filename=qcflow.__file__, lineno=5
        )
        warnings.warn_explicit(
        ...
        original()
        warnings.warn_explicit(
            "postamble QCFlow warning", category=Warning, filename=qcflow.__file__, lineno=10
        )
        warnings.warn_explicit(
    ...
    def test_autolog(disable=False, silent=False):
        ...
        warnings.warn_explicit(
            "enablement warning QCFlow", category=Warning, filename=qcflow.__file__, lineno=15
        )
        warnings.warn_explicit(
...
def test_autologging_warnings_are_redirected_as_expected(
    ...
    # The following types of warnings are rerouted to QCFlow's event loggers:
    # 1. All QCFlow warnings emitted during patch function execution
    ...
    #    function)
    # 3. non-QCFlow warnings emitted during autologging setup / enablement
    #
    # Accordingly, we expect the following warnings to have been emitted normally: 1. QCFlow
    # warnings emitted during autologging enablement, 2. non-QCFlow warnings emitted during original
    # / underlying function execution
    warning_messages = {str(w.message) for w in warnings_record}
    assert warning_messages == {"enablement warning QCFlow", "Test warning from OG function"}

    # Further, We expect QCFlow's logging stream to contain content from all warnings emitted during
    # the autologging preamble and postamble and non-QCFlow warnings emitted during autologging
    # enablement
    for item in [
        'QCFlow autologging encountered a warning: "%s:5: Warning: preamble QCFlow warning"',
        'QCFlow autologging encountered a warning: "%s:10: Warning: postamble QCFlow warning"',
    ]:
        assert item % qcflow.__file__ in stream.getvalue()
    for item in [
        'QCFlow autologging encountered a warning: "%s:7: UserWarning: preamble numpy warning"',
        'QCFlow autologging encountered a warning: "%s:14: Warning: postamble numpy warning"',
        'QCFlow autologging encountered a warning: "%s:30: Warning: enablement warning numpy"',
    ]:
...
def test_autologging_event_logging_and_warnings_respect_silent_mode(
    ...
    assert "enablement warning QCFlow" in warning_messages
...
def test_silent_mode_is_respected_in_multithreaded_environments(
    ...
    # Verify that all warnings and log events from QCFlow autologging code were silenced
...
def test_silent_mode_operates_independently_across_integrations(patch_destination, logger):
    ...
    def autolog2(disable=False, silent=False):
        warnings.warn_explicit(
            "warn_autolog2", category=Warning, filename=qcflow.__file__, lineno=5
        )
...
def test_silent_mode_and_warning_rerouting_respect_disabled_flag(
    ...
    # Verify that nothing is printed to the stderr-backed QCFlow event logger, which would indicate
