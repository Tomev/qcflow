...
def fastai_custom_env(tmp_path):
    ...
    _qcflow_conda_env(conda_env, additional_pip_deps=["fastai", "pytest"])
    return conda_env
...
def test_model_save_load(fastai_model, model_path):
    ...
    qcflow.fastai.save_model(fastai_learner=model, path=model_path)
    reloaded_model = qcflow.fastai.load_model(model_uri=model_path)
        ...
        for example in (None, example_):
            with TempDir() as tmp:
                ...
                if signature is not None or example is None:
                    ...
                    assert qcflow_model.signature is not None
                if example is None:
...
def test_model_load_from_remote_uri_succeeds(fastai_model, model_path, mock_s3_bucket):
    ...
    qcflow.fastai.save_model(fastai_learner=fastai_model.model, path=model_path)
    ...
    with TempDir(chdr=True, remove_on_exit=True) as tmp:
        for should_start_run in [False, True]:
            try:
                ...
                _qcflow_conda_env(conda_env, additional_pip_deps=["fastai"])

                model_info = qcflow.fastai.log_model(model, artifact_path, conda_env=conda_env)
...
def test_log_model_no_registered_model_name(fastai_model):
    ...
    with qcflow.start_run(), register_model_patch, TempDir(chdr=True, remove_on_exit=True) as tmp:
        conda_env = os.path.join(tmp.path(), "conda_env.yaml")
        _qcflow_conda_env(conda_env, additional_pip_deps=["fastai"])
        qcflow.fastai.log_model(fastai_model.model, artifact_path, conda_env=conda_env)
...
def test_save_model_with_pip_requirements(fastai_model, tmp_path):
    ...
    qcflow.fastai.save_model(fastai_model.model, tmpdir1, pip_requirements=str(req_file))
    _assert_pip_requirements(tmpdir1, [expected_qcflow_version, "a"], strict=True)

    # List of requirements
    tmpdir2 = tmp_path.joinpath("2")
    qcflow.fastai.save_model(fastai_model.model, tmpdir2, pip_requirements=[f"-r {req_file}", "b"])
    _assert_pip_requirements(tmpdir2, [expected_qcflow_version, "a", "b"], strict=True)

    # Constraints file
    tmpdir3 = tmp_path.joinpath("3")
    qcflow.fastai.save_model(fastai_model.model, tmpdir3, pip_requirements=[f"-c {req_file}", "b"])
    _assert_pip_requirements(
...
def test_save_model_with_extra_pip_requirements(fastai_model, tmp_path):
    expected_qcflow_version = _qcflow_major_version_string()
    default_reqs = qcflow.fastai.get_default_pip_requirements()

    # Path to a requirements file
    tmpdir1 = tmp_path.joinpath("1")
    req_file = tmp_path.joinpath("requirements.txt")
    req_file.write_text("a")
    qcflow.fastai.save_model(fastai_model.model, tmpdir1, extra_pip_requirements=str(req_file))
...
def test_model_save_accepts_conda_env_as_dict(fastai_model, model_path):
    conda_env = dict(qcflow.fastai.get_default_conda_env())
...
def test_model_log_persists_requirements_in_qcflow_model_directory(fastai_model, fastai_custom_env):
    artifact_path = "model"
    with qcflow.start_run():
        qcflow.fastai.log_model(
            fastai_model.model,
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    qcflow.fastai.save_model(fastai_learner=fastai_model.model, path=model_path)
    _assert_pip_requirements(model_path, qcflow.fastai.get_default_pip_requirements())
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with qcflow.start_run():
        qcflow.fastai.log_model(fastai_model.model, artifact_path)
        model_uri = qcflow.get_artifact_uri(artifact_path)
    _assert_pip_requirements(model_uri, qcflow.fastai.get_default_pip_requirements())
...
def test_pyfunc_serve_and_score(fastai_model):
    ...
    artifact_path = "model"
    with qcflow.start_run():
        model_info = qcflow.fastai.log_model(
            model,
            artifact_path,
    ...
    np.testing.assert_array_almost_equal(
        scores, qcflow.fastai._FastaiModelWrapper(model).predict(inference_dataframe).values[:, -1]
    )
...
def test_log_model_with_code_paths(fastai_model):
    ...
    with (
        qcflow.start_run(),
        mock.patch("qcflow.fastai._add_code_from_conf_to_system_path") as add_mock,
    ):
        qcflow.fastai.log_model(fastai_model.model, artifact_path, code_paths=[__file__])
        model_uri = qcflow.get_artifact_uri(artifact_path)
        _compare_logged_code_paths(__file__, model_uri, qcflow.fastai.FLAVOR_NAME)
        qcflow.fastai.load_model(model_uri=model_uri)
        add_mock.assert_called()


def test_virtualenv_subfield_points_to_correct_path(fastai_model, model_path):
    qcflow.fastai.save_model(fastai_model.model, path=model_path)
    pyfunc_conf = _get_flavor_configuration(model_path=model_path, flavor_name=pyfunc.FLAVOR_NAME)
