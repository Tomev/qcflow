...
def runs_cancel_mock():
    ...
    with mock.patch(
        "qcflow.projects.databricks.DatabricksJobRunner.jobs_runs_cancel"
    ) as runs_cancel_mock:
...
def runs_submit_mock():
    ...
    with mock.patch(
        "qcflow.projects.databricks.DatabricksJobRunner._jobs_runs_submit",
...
def runs_get_mock():
    ...
    with mock.patch(
        "qcflow.projects.databricks.DatabricksJobRunner.jobs_runs_get"
    ) as runs_get_mock:
        yield runs_get_mock


@pytest.fixture
def databricks_cluster_qcflow_run_cmd_mock():
    """Mocks the Jobs Runs Get API request"""
    with mock.patch(
        "qcflow.projects.databricks._get_cluster_qcflow_run_cmd"
    ) as qcflow_run_cmd_mock:
        yield qcflow_run_cmd_mock
...
def upload_to_dbfs_mock(dbfs_root_mock):
    ...
    with mock.patch.object(
        qcflow.projects.databricks.DatabricksJobRunner, "_upload_to_dbfs", new=upload_mock_fn
    ) as upload_mock:
        yield upload_mock
...
def dbfs_path_exists_mock(dbfs_root_mock):
    with mock.patch(
        "qcflow.projects.databricks.DatabricksJobRunner._dbfs_path_exists"
    ) as path_exists_mock:
...
def before_run_validations_mock():
    with mock.patch("qcflow.projects.databricks.before_run_validations"):
        yield
...
def test_upload_existing_project_to_dbfs(dbfs_path_exists_mock):
    ...
    with mock.patch(
        "qcflow.projects.databricks.DatabricksJobRunner._upload_to_dbfs"
...
def test_run_databricks_validations(
    ...
    with mock.patch(
        "qcflow.projects.databricks.DatabricksJobRunner._databricks_api_request"
    ) as db_api_req_mock:
        # Test bad tracking URI
        qcflow.set_tracking_uri(tmp_path.as_uri())
        with pytest.raises(ExecutionException, match="QCFlow tracking URI must be of"):
...
@pytest.mark.usefixtures(
    ...
    "runs_cancel_mock",
    "dbfs_mocks",
    "databricks_cluster_qcflow_run_cmd_mock",
)
def test_run_databricks(
    ...
    qcflow.set_tracking_uri("databricks")
    # Test that QCFlow gets the correct run status when performing a Databricks run
    for run_succeeded, expect_status in [(True, RunStatus.FINISHED), (False, RunStatus.FAILED)]:
        ...
        assert databricks_cluster_qcflow_run_cmd_mock.call_count == 1
        tags = {}
        for call_args, _ in set_tag_mock.call_args_list:
            tags[call_args[1]] = call_args[2]
        assert tags[QCFLOW_DATABRICKS_RUN_URL] == "test_url"
        assert tags[QCFLOW_DATABRICKS_SHELL_JOB_RUN_ID] == "-1"
        assert tags[QCFLOW_DATABRICKS_WEBAPP_URL] == "https://test-host"
        set_tag_mock.reset_mock()
        runs_submit_mock.reset_mock()
        databricks_cluster_qcflow_run_cmd_mock.reset_mock()
...
def test_run_databricks_cancel(
    ...
    # Test that QCFlow properly handles Databricks run cancellation. We mock the result of
    ...
    runs_get_mock.return_value = mock_runs_get_result(succeeded=False)
    with pytest.raises(qcflow.projects.ExecutionException, match=r"Run \(ID '.+'\) failed"):
        run_databricks_project(cluster_spec_mock, synchronous=True)


def test_get_tracking_uri_for_run(monkeypatch):
    qcflow.set_tracking_uri("http://some-uri")
    assert databricks._get_tracking_uri_for_run() == "http://some-uri"
    qcflow.set_tracking_uri("databricks://profile")
    assert databricks._get_tracking_uri_for_run() == "databricks"
    qcflow.set_tracking_uri(None)
    monkeypatch.setenv(QCFLOW_TRACKING_URI.name, "http://some-uri")
    assert qcflow.tracking._tracking_service.utils.get_tracking_uri() == "http://some-uri"
...
def test_run_databricks_failed(_):
    ...
    with mock.patch(
        "qcflow.utils.rest_utils.http_request", return_value=mock.Mock(text=text, status_code=400)
    ):
...
def test_run_databricks_generates_valid_qcflow_run_cmd():
    cmd = _get_cluster_qcflow_run_cmd(
    ...
    assert cmd[0] == "qcflow"
    with mock.patch("qcflow.projects.run"):
