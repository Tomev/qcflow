...
def statsmodels_custom_env(tmp_path):
    ...
    _qcflow_conda_env(conda_env, additional_pip_deps=["pytest", "statsmodels"])
    return conda_env
...
def _test_model_save_load(statsmodels_model, model_path, *predict_args):
    qcflow.statsmodels.save_model(statsmodels_model=statsmodels_model.model, path=model_path)
    reloaded_model = qcflow.statsmodels.load_model(model_uri=model_path)
...
def _test_model_log(statsmodels_model, model_path, *predict_args):
    ...
    with TempDir(chdr=True, remove_on_exit=True) as tmp:
        try:
            ...
            _qcflow_conda_env(conda_env, additional_pip_deps=["statsmodels"])

            model_info = qcflow.statsmodels.log_model(model, artifact_path, conda_env=conda_env)
            model_uri = f"runs:/{qcflow.active_run().info.run_id}/{artifact_path}"
            assert model_info.model_uri == model_uri

            reloaded_model = qcflow.statsmodels.load_model(model_uri=model_uri)
...
def test_model_load_from_remote_uri_succeeds(model_path, mock_s3_bucket):
    ...
    qcflow.statsmodels.save_model(statsmodels_model=model, path=model_path)
...
def test_log_model_no_registered_model_name():
    ...
    with qcflow.start_run(), register_model_patch, TempDir(chdr=True, remove_on_exit=True) as tmp:
        conda_env = os.path.join(tmp.path(), "conda_env.yaml")
        _qcflow_conda_env(conda_env, additional_pip_deps=["statsmodels"])
        qcflow.statsmodels.log_model(ols.model, artifact_path, conda_env=conda_env)
...
def test_log_model_with_pip_requirements(tmp_path):
    ...
    with qcflow.start_run():
        qcflow.statsmodels.log_model(ols.model, "model", pip_requirements=str(req_file))
        _assert_pip_requirements(
    ...
    with qcflow.start_run():
        qcflow.statsmodels.log_model(ols.model, "model", pip_requirements=[f"-r {req_file}", "b"])
        _assert_pip_requirements(
    ...
    # Constraints file
    with qcflow.start_run():
        qcflow.statsmodels.log_model(ols.model, "model", pip_requirements=[f"-c {req_file}", "b"])
        _assert_pip_requirements(
...
def test_log_model_with_extra_pip_requirements(tmp_path):
    expected_qcflow_version = _qcflow_major_version_string()
    ols = ols_model()
    default_reqs = qcflow.statsmodels.get_default_pip_requirements()

    # Path to a requirements file
    req_file = tmp_path.joinpath("requirements.txt")
    req_file.write_text("a")
    with qcflow.start_run():
        qcflow.statsmodels.log_model(ols.model, "model", extra_pip_requirements=str(req_file))
        _assert_pip_requirements(
...
def test_model_save_accepts_conda_env_as_dict(model_path):
    ols = ols_model()
    conda_env = dict(qcflow.statsmodels.get_default_conda_env())
    conda_env["dependencies"].append("pytest")
    qcflow.statsmodels.save_model(statsmodels_model=ols.model, path=model_path, conda_env=conda_env)
...
def test_model_log_persists_specified_conda_env_in_qcflow_model_directory(statsmodels_custom_env):
    ols = ols_model()
    artifact_path = "model"
    with qcflow.start_run():
        qcflow.statsmodels.log_model(
...
def test_model_log_persists_requirements_in_qcflow_model_directory(statsmodels_custom_env):
    ols = ols_model()
    artifact_path = "model"
    with qcflow.start_run():
        qcflow.statsmodels.log_model(
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    ols = ols_model()
    qcflow.statsmodels.save_model(statsmodels_model=ols.model, path=model_path)
    _assert_pip_requirements(model_path, qcflow.statsmodels.get_default_pip_requirements())
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies():
    ...
    artifact_path = "model"
    with qcflow.start_run():
        qcflow.statsmodels.log_model(ols.model, artifact_path)
        model_uri = qcflow.get_artifact_uri(artifact_path)
    _assert_pip_requirements(model_uri, qcflow.statsmodels.get_default_pip_requirements())
...
def test_pyfunc_serve_and_score():
    ...
    artifact_path = "model"
    with qcflow.start_run():
        model_info = qcflow.statsmodels.log_model(
...
def test_log_model_with_code_paths():
    ...
    with (
        qcflow.start_run(),
        mock.patch("qcflow.statsmodels._add_code_from_conf_to_system_path") as add_mock,
    ):
        qcflow.statsmodels.log_model(ols.model, artifact_path, code_paths=[__file__])
        model_uri = qcflow.get_artifact_uri(artifact_path)
        _compare_logged_code_paths(__file__, model_uri, qcflow.statsmodels.FLAVOR_NAME)
        qcflow.statsmodels.load_model(model_uri)
        add_mock.assert_called()


def test_virtualenv_subfield_points_to_correct_path(model_path):
    ols = ols_model()
    qcflow.statsmodels.save_model(ols.model, path=model_path)
    pyfunc_conf = _get_flavor_configuration(model_path=model_path, flavor_name=pyfunc.FLAVOR_NAME)
...
def test_model_log_with_signature_inference():
    ...
    with qcflow.start_run():
        qcflow.statsmodels.log_model(model, artifact_path, input_example=example)
        model_uri = qcflow.get_artifact_uri(artifact_path)
