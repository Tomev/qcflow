...
def replace_qcflow_with_dev_version(yml_path: Path) -> None:
    old_src = yml_path.read_text()
    qcflow_dir = Path(qcflow.__path__[0]).parent
    new_src = re.sub(r"- qcflow.*\n", f"- {qcflow_dir}\n", old_src)
    yml_path.write_text(new_src)


@pytest.fixture(autouse=True)
def clean_up_qcflow_virtual_environments():
    yield

    for path in Path(_get_qcflow_virtualenv_root()).iterdir():
        if path.is_dir():
            shutil.rmtree(path)
...
def test_qcflow_run_example(directory, params, tmp_path):
    ...
    qcflow.set_tracking_uri(random_tmp_path.joinpath("mlruns").as_uri())
    ...
    replace_qcflow_with_dev_version(python_env_path)
...
@pytest.mark.parametrize(
    ("directory", "command"),
    [
        ("docker", ["docker", "build", "-t", "qcflow-docker-example", "-f", "Dockerfile", "."]),
        ...
        ("quickstart", [sys.executable, "qcflow_tracking.py"]),
...
def test_command_example(directory, command):
    ...
    assert os.environ.get("QCFLOW_HOME") is not None
