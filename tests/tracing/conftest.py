...
def mock_upload_trace_data():
    with (
        mock.patch(
            "qcflow.tracking._tracking_service.client.TrackingServiceClient.end_trace",
...
def mock_store(monkeypatch):
    ...
    """
    store = qcflow.tracking._tracking_service.utils._get_store()
...
def databricks_tracking_uri():
    with mock.patch(
        "qcflow.tracking._tracking_service.utils.get_tracking_uri", return_value="databricks"
    ):
        yield
...
def async_logging_enabled(request, monkeypatch):
    monkeypatch.setenv(QCFLOW_ENABLE_ASYNC_LOGGING.name, str(request.param))
