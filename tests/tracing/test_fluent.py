...
from qcflow.tracing.provider import _get_trace_exporter, _get_tracer
...
class DefaultTestModel:
    ...
    def predict(self, x, y):
        z = x + y
        z = self.add_one(z)
        z = qcflow.trace(self.square)(z)
        return z  # noqa: RET504

    @qcflow.trace(span_type=SpanType.LLM, name="add_one_with_custom_name", attributes={"delta": 1})
    def add_one(self, z):
        return z + 1
...
class DefaultAsyncTestModel:
    ...
    async def predict(self, x, y):
        ...
        z = await qcflow.trace(self.square)(z)
        return z  # noqa: RET504
...
def test_trace(wrap_sync_func, with_active_run, async_logging_enabled):
    ...
    assert child_span_2.attributes == {
        "qcflow.traceRequestId": trace.info.request_id,
        "qcflow.spanFunctionName": "square",
        "qcflow.spanType": "UNKNOWN",
        "qcflow.spanInputs": {"t": 8},
        "qcflow.spanOutputs": 64,
    }
...
def test_trace_in_databricks_model_serving(
    ...
    class TestModel:
        ...
        def predict(self, x, y):
            z = x + y
            z = self.add_one(z)
            with qcflow.start_span(name="square") as span:
                z = self.square(z)
                span.add_event(SpanEvent("event", 0, attributes={"foo": "bar"}))
            return z

        @qcflow.trace(span_type=SpanType.LLM, name="custom", attributes={"delta": 1})
        def add_one(self, z):
            return z + 1
    ...
    assert root_span.attributes == {
        "qcflow.traceRequestId": databricks_request_id,
        "qcflow.spanType": SpanType.UNKNOWN,
        "qcflow.spanFunctionName": "predict",
        "qcflow.spanInputs": {"x": 2, "y": 5},
        "qcflow.spanOutputs": 64,
    }
...
def test_trace_in_model_evaluation(mock_store, monkeypatch, async_logging_enabled):
    ...
    trace = qcflow.get_trace(request_id_1)
    assert trace.info.request_metadata[TraceMetadataKey.SOURCE_RUN] == run_id
    ...
    trace = qcflow.get_trace(request_id_2)
    assert trace.info.request_metadata[TraceMetadataKey.SOURCE_RUN] == run_id
...
def test_trace_ignore_exception_from_tracing_logic(monkeypatch, async_logging_enabled):
    ...
    with mock.patch("qcflow.tracing.provider._get_tracer", side_effect=ValueError("Some error")):
    ...
    with mock.patch(
        "qcflow.tracing.fluent.capture_function_input_args", side_effect=ValueError("Some error")
    ) as mock_input_args:
        output = model.predict(2, 5)
...
def test_trace_skip_resolving_unrelated_tags_to_traces():
    with mock.patch("qcflow.tracking.context.registry.DatabricksRepoRunContext") as mock_context:
    ...
    class TestModel:
        def predict(self, x, y):
            with qcflow.start_span(name="root_span") as root_span:
                ...
                with qcflow.start_span(name="child_span", span_type=SpanType.LLM) as child_span:
        ...
        def square(self, t):
            with qcflow.start_span(name="child_span") as span:
                span.set_inputs({"t": t})
                res = t**2
                time.sleep(0.1)
...
def test_start_span_context_manager_with_imperative_apis(async_logging_enabled):
    ...
    # (via QCFlow client) are correctly linked together. This usage is not recommended but
    ...
    class TestModel:
        ...
        def predict(self, x, y):
            with qcflow.start_span(name="root_span") as root_span:
                ...
                child_span = self._qcflow_client.start_span(
                    name="child_span_1",
...
def test_qcflow_trace_isolated_from_other_otel_processors():
    ...
    # Create QCFlow trace
    with qcflow.start_span(name="qcflow_span"):
        pass

    # Create non-QCFlow trace
    tracer = trace.get_tracer(__name__)
    with tracer.start_as_current_span("non_qcflow_span"):
        pass

    # QCFlow only processes spans created with QCFlow APIs
    assert len(TRACE_BUFFER) == 1
    assert qcflow.get_last_active_trace().data.spans[0].name == "qcflow_span"

    # Other spans are processed by the other processor
    assert len(other_exporter.exported_spans) == 1
    assert other_exporter.exported_spans[0].name == "non_qcflow_span"


@mock.patch("qcflow.tracing.export.qcflow.get_display_handler")
def test_get_trace(mock_get_display_handler):
    ...
    trace_in_memory = qcflow.get_trace(request_id)
    ...
    TRACE_BUFFER.clear()
    trace_from_backend = qcflow.get_trace(request_id)
    assert trace.info.request_id == trace_from_backend.info.request_id
    mock_get_display_handler.assert_not_called()

    # If not found, return None with warning
    with mock.patch("qcflow.tracing.fluent._logger") as mock_logger:
        assert qcflow.get_trace("not_found") is None
...
def test_test_search_traces_empty(mock_client):
    ...
    traces = qcflow.search_traces(extract_fields=["foo.inputs.bar"])
...
def test_search_traces_with_pagination(mock_client):
    ...
    traces = qcflow.search_traces(experiment_ids=["1"])
...
def test_search_traces_with_default_experiment_id(mock_client):
    mock_client.search_traces.return_value = PagedList([], token=None)
    with mock.patch("qcflow.tracing.fluent._get_experiment_id", return_value="123"):
...
def test_search_traces_yields_expected_dataframe_contents(monkeypatch):
    ...
    for _ in range(10):
        ...
        trace = client.get_trace(qcflow.get_last_active_trace().info.request_id)
        expected_traces.append(trace)

    df = qcflow.search_traces(max_results=10, order_by=["timestamp ASC"])
...
def test_search_traces_with_non_dict_span_inputs_outputs(monkeypatch):
    with qcflow.start_span(name="non_dict_span") as span:
        span.set_inputs(["a", "b"])
...
def test_search_traces_with_multiple_spans_with_same_name(monkeypatch):
    class TestModel:
        @qcflow.trace(name="duplicate_name")
        def predict(self, x, y):
            z = x + y
            z = self.add_one(z)
            z = qcflow.trace(self.square)(z)
        ...
        @qcflow.trace(span_type=SpanType.LLM, name="duplicate_name", attributes={"delta": 1})
        def add_one(self, z):
            return z + 1
...
def test_search_traces_span_and_field_name_with_dot():
    with qcflow.start_span(name="span.name") as span:
...
def test_search_traces_with_span_name(monkeypatch):
    class TestModel:
        ...
        @qcflow.trace(span_type=SpanType.LLM, name="span.invalidname", attributes={"delta": 1})
        def add_one(self, z):
            return z + 1
...
def test_search_traces_with_run_id():
    ...
    traces = qcflow.search_traces(run_id=run1.info.run_id)
...
def test_update_current_trace():
    @qcflow.trace
    def f(x):
        qcflow.update_current_trace(tags={"fruit": "apple", "animal": "dog"})
        return g(x) + 1

    @qcflow.trace
    def g(y):
        with qcflow.start_span():
            qcflow.update_current_trace(tags={"fruit": "orange", "vegetable": "carrot"})
            return y * 2
    ...
    tags = {k: v for k, v in trace.info.tags.items() if not k.startswith("qcflow.")}
...
def test_non_ascii_characters_not_encoded_as_unicode():
    ...
    artifact_location = local_file_uri_to_path(trace.info.tags["qcflow.artifactLocation"])
...
def test_export_to_otel_collector(otel_collector, mock_client, monkeypatch):
    ...
    # Traces should not be logged to QCFlow
...
def test_add_trace_merge_tags():
    ...
    with qcflow.start_span(name="parent") as span:
        ...
        qcflow.add_trace(Trace.from_dict(_SAMPLE_REMOTE_TRACE))

    trace = qcflow.get_last_active_trace()
    custom_tags = {k: v for k, v in trace.info.tags.items() if not k.startswith("qcflow.")}
    assert custom_tags == {
        "fruit": "apple",
        "vegetable": "carrot",
