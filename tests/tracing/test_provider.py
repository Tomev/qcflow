...
def test_disable_enable_tracing():
    ...
    # enable() / disable() should only raise QCFlowTracingException
    with mock.patch(
...
def test_trace_disabled_decorator(enabled_initially):
    ...
    with mock.patch(
        "qcflow.tracing.provider.disable", side_effect=QCFlowTracingException("error")
    ) as disable_mock:
        assert test_fn() == 0
        assert call_count == 3
    ...
    with mock.patch(
        "qcflow.tracing.provider.enable", side_effect=QCFlowTracingException("error")
    ) as enable_mock:
        assert test_fn() == 0
        assert call_count == 4
...
def test_is_tracing_enabled():
    ...
    # is_tracing_enabled() should only raise QCFlowTracingException
    with mock.patch(
