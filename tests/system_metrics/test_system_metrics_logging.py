...
def disable_system_metrics_logging():
    ...
    qcflow.set_system_metrics_sampling_interval(None)
    qcflow.set_system_metrics_samples_before_logging(None)
    qcflow.set_system_metrics_node_id(None)
...
def test_manual_system_metrics_monitor():
    ...
    with qcflow.start_run(log_system_metrics=False) as run:
    ...
    qcflow_run = qcflow.get_run(run.info.run_id)
    metrics = qcflow_run.data.metrics
...
def test_automatic_system_metrics_monitor():
    ...
    qcflow.set_system_metrics_sampling_interval(0.2)
    qcflow.set_system_metrics_samples_before_logging(2)
...
def test_automatic_system_metrics_monitor_resume_existing_run():
    ...
    with qcflow.start_run(run.info.run_id) as run:
        time.sleep(2)
    qcflow_run = qcflow.get_run(run.info.run_id)
    metrics = qcflow_run.data.metrics
...
def test_system_metrics_monitor_with_multi_node():
    ...
    for node_id in node_ids:
        qcflow.set_system_metrics_node_id(node_id)
        with qcflow.start_run(run_id=run_id, log_system_metrics=True):
            wait_for_condition(
                lambda: any(
                    k.startswith(f"system/{node_id}/")
                    for k in qcflow.get_run(run_id).data.metrics.keys()
                )
            )

    qcflow_run = qcflow.get_run(run_id)
    metrics = qcflow_run.data.metrics
