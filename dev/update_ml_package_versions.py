"""
A script to update the maximum package versions in 'qcflow/ml-package-versions.yml'.
...
def update_ml_package_versions_py(config_path):
    with open(config_path) as f:
        ...
        dst = Path("qcflow", "ml_package_versions.py")
        Path(dst).write_text(
            f"""\
...
def parse_args():
    parser = argparse.ArgumentParser(description="Update QCFlow package versions")
    parser.add_argument(
...
def update(skip_yml=False):
    yml_path = "qcflow/ml-package-versions.yml"
