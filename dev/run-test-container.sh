...
DOCKER_BUILDKIT=1 docker build -f "${ROOT_DIR}/dev/Dockerfile.test" -t qcflow-test-env "$ROOT_DIR" --build-arg MINIFORG_FILENAME="$MINIFORG_FILENAME"
docker container rm -f qcflow-test 2>/dev/null
docker run \
  -v "${ROOT_DIR}"/tests:/app/tests \
  -v "${ROOT_DIR}"/qcflow:/app/qcflow \
...
docker container rm qcflow-test
