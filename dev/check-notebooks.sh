...
if grep -In $TRACE_UI_SRC --include="*.ipynb" "$@"; then
    echo -e "\nError: Found the QCFlow Trace UI iframe in the above notebooks."
    echo "The trace UI in cell outputs will not render correctly in previews or the website"
    echo "Please run \`qcflow.tracing.disable_notebook_display()\` and rerun the cell to remove the iframe."
    exit 1
else
  exit 0
fi
