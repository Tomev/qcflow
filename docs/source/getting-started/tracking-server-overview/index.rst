...
In this guide we will walk you through how to view your QCFlow experiment results with different types of 
tracking server configurations. At a high level, there are 3 ways to view your QCFlow experiments:

* **[Method 1]** Start your own QCFlow server.
...
Overall **Method 2 - free hosted tracking server** is the simplest way to get started with QCFlow, but please
...
A hosted tracking server is the simplest way to store and view QCFlow experiments, but it is not suitable for 
...
you can use a local hosted QCFlow server to store and view your experiments. To do so, there are two steps:

* Start your QCFlow server.
* Connect QCFlow session to the local QCFlow server IP by :py:func:`qcflow.set_tracking_uri()`.


Start a Local QCFlow Server
^^^^^^^^^^^^^^^^^^^^^^^^^^^

If you don't have QCFlow installed, please run the command below to install it:

.. code-block:: bash

    $ pip install qcflow

The installation of QCFlow includes the QCFlow CLI tool, so you can start a local QCFlow server with UI 
...
.. code-block:: bash
    ...
    (qcflow) [master][~/Documents/qcflow_team/qcflow]$ qcflow ui
...
Opening the URL of the QCFlow tracking server in your browser will bring you to the QCFlow UI. The image below is from the open source version of the QCFlow UI, 
which is a bit different from the QCFlow UI on Databricks CE. Below is a screenshot of the landing page:

.. image:: ../../_static/images/quickstart/tracking-server-overview/qcflow-localhost-landing-page.png
    :width: 800px
    :align: center
    :alt: Landing page of OSS QCFlow server

.. note::

  It's also possible to deploy your own QCFlow server on cloud platforms, but it is out of the scope of this guide.


Connect QCFlow Session to Your Server
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Now that the server is spun up, let's connect our QCFlow session to the local server. This is very 
...
.. code-block:: python
  ...
  qcflow.set_experiment("check-localhost-connection")

  with qcflow.start_run():
      qcflow.log_metric("foo", 1)
      qcflow.log_metric("bar", 2)


Putting it together you can copy the following code to your editor and save it as `log_qcflow_with_localhost.py`:
...
.. code-block:: bash
  ...
  $ python log_qcflow_with_localhost.py


View Experiment on Your QCFlow Server
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. image:: ../../_static/images/quickstart/tracking-server-overview/qcflow-localhost-experiment-view.png
    :width: 800px
    :align: center
    :alt: Experiment view of OSS QCFlow server
...
.. image:: ../../_static/images/quickstart/tracking-server-overview/qcflow-localhost-run-view.png
    :width: 800px
    :align: center
    :alt: Run view of OSS QCFlow server

Conclusion
^^^^^^^^^^^

That's all about how to start your own QCFlow server and view your experiments. Please see the pros and cons
...
* **Pros**
  ...
  * You can even customize your UI by forking the QCFlow repo and modify the UI code.
...
charge with Databricks. You can use Databricks CE to store and view your QCFlow experiments without 
being charged.

To use Databricks CE to store and view our QCFlow experiments, basically we need to:

* Create a free Databricks CE account.
* Set up Databricks CE authentication in our dev environment.
* Connect to Databricks CE in our QCFlow experiment session.

Then the experiment results will be automatically sent to Databricks CE, where you can view it in 
QCFlow experiment UI. Now let's look at the code.
...
.. code-block:: bash
  ...
  %pip install -q qcflow databricks-sdk

Set Up Authentication of Databricks CE
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

To set up Databricks CE authentication, we can use the API :py:func:`qcflow.login()`, which will prompt you for required information:
...
.. code-block:: text
  ...
  2023/10/25 22:59:27 ERROR qcflow.utils.credentials: Failed to sign in Databricks: default auth: cannot configure default credentials
  Databricks Host (should begin with https://): https://community.cloud.databricks.com/
  Username: weirdmouse@gmail.com
  Password: ··········
  2023/10/25 22:59:38 INFO qcflow.utils.credentials: Succesfully signed in Databricks!


Connect QCFlow Session to Databricks CE
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

We have set up the credentials, now we need to tell QCFlow to send the data into Databricks CE. 
To do so, we will use ``qcflow.set_tracking_uri("databricks")`` to port QCFlow to Databricks CE. Basically 
...
Now you are ready to go! Let's try starting an QCFlow experiment and log some dummy metrics and view it in our UI.

.. code-block:: python

  qcflow.set_experiment("/check-databricks-connection")
...
.. code-block:: text
  ...
  2023/10/25 23:15:42 INFO qcflow.tracking.fluent: Experiment with name '/check-databricks-ce-connection' does not exist. Creating a new experiment.
...
.. image:: ../../_static/images/quickstart/tracking-server-overview/databricks-ce-landing-page.png 
    :width: 800px
    :align: center
    :alt: Landing page of Databricks QCFlow server
...
.. image:: ../../_static/images/quickstart/tracking-server-overview/databricks-ce-experiment-view.png
    :width: 800px
    :align: center
    :alt: Experiment view of Databricks QCFlow server
...
* **Pros**
  ...
  * Effortless setup.
  * Free. 
  * Good for collaboration, e.g., you can share your QCFlow experiment with your teammates easily.
...
Databricks or Microsoft AzureML. If you use Databricks, QCFlow experiment will log your model into the Databricks 
QCFlow server, and you can register your model then serve your model by a few clicks. Serving feature 
...
`the doc here <https://learn.microsoft.com/en-us/azure/machine-learning/how-to-use-qcflow-cli-runs?view=azureml-api-2&tabs=interactive%2Ccli>`_
