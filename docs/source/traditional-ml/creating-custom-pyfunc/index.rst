Building Custom Python Function Models with QCFlow
==================================================

QCFlow offers a wide range of pre-defined model flavors, but there are instances where you'd want to go 
...
- **Named Model Flavors**: Before we dive into the custom territory, it's essential to understand the existing named flavors in QCFlow. These pre-defined flavors simplify model tracking and deployment, but they might not cover every use case.
...
- **Custom PyFuncs Demystified**: What exactly is a custom PyFunc? How is it different from the named flavors, and when would you want to use one? We'll cover:
    ...
    - **Unsupported Libraries**: Maybe you're using a niche or newly-released ML library that QCFlow doesn't support yet. No worries, custom PyFuncs have you covered.
...
- **Tackling Unsupported Libraries**: Stepping up the complexity, we'll demonstrate how to integrate a model from an unsupported library into QCFlow using custom PyFuncs.
...
By the end of this tutorial, you'll have a clear understanding of how to leverage custom PyFuncs in QCFlow to cater to specialized needs, 
ensuring flexibility without compromising on the ease of use.

.. toctree::
    :maxdepth: 1

    Models, Flavors, and PyFuncs in QCFlow <part1-named-flavors>
