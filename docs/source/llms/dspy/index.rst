...
.. figure:: ../../_static/images/llms/dspy/dspy-integration-architecture.png
    :alt: Overview of DSPy and QCFlow integration
    :width: 70%
    :align: center


Why use DSPy with QCFlow?
-------------------------------

The native integration of the DSPy library with QCFlow helps users manage the development lifecycle with DSPy. The following are some of the key benefits of using DSPy with QCFlow:

* `QCFlow Tracking <../../tracking.html>`_ allows you to track your DSPy program's training and execution. With the QCFlow APIs, you can log a variety of artifacts and organize training runs, thereby increasing visibility into your model performance.  
* `QCFlow Model <../../models.html>`_ packages your compiled DSPy program along with its dependency versions, input and output interfaces and other essential metadata. This allows you to deploy your compiled DSPy program with ease, knowing that the environment is consistent across different stages of the ML lifecycle.
* `QCFlow Evaluate <../llm-evaluate/index.html>`_ provides native capabilities within QCFlow to evaluate GenAI applications. This capability facilitates the efficient assessment of inference results from your DSPy compiled program, ensuring robust performance analytics and facilitating quick iterations.
...
In this introductory tutorial, you will learn the most fundamental components of DSPy and how to leverage the integration with QCFlow to store, retrieve, and 
use a DSPy program. 
        ...
        <article class="simple-grid">
            <div class="simple-card">
                <a href="notebooks/dspy_quickstart.html">
                    ...
                    <p>
                        Get started with QCFlow and DSPy by exploring the simplest possible configuration of a DSPy program.
                    </p>
                </a>
            </div>
        </article>
...
Saving and Loading DSPy Program in QCFlow Experiment
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
the DSPy and QCFlow integration. With DSPy, you can create complex agentic logic via a module or 
set of modules.

.. code-block:: shell

    pip install qcflow dspy -U
...
.. tip::
    ...
    module. QCFlow will natively supports logging both compiled and uncompiled DSPy modules. Above 
...
Logging the Program to QCFlow
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

You can log the ``dspy.Module`` object to an QCFlow run using the 
:py:func:`qcflow.dspy.log_model` function.

We will also specify a `model signature <https://qcflow.org/docs/latest/model/signatures.html#input-example>`_. 
An QCFlow model signature defines the expected schema for model inputs and outputs, ensuring 
...
.. figure:: ../../_static/images/llms/dspy/dspy-artifacts.png
    :alt: QCFlow artifacts for the DSPy program 
    :width: 80%
    :align: center

Loading the Module for inference
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The saved module can be loaded back for inference using the :py:func:`qcflow.pyfunc.load_model` function. This function
gives an QCFlow Python Model backed by the DSPy module.

.. code-block:: python

    import qcflow

    # Load the model as an QCFlow PythonModel
...
To load the DSPy program itself back instead of the PyFunc-wrapped model, use the :py:func:`qcflow.dspy.load_model` function.

.. code-block:: python

    model = qcflow.dspy.load_model(model_uri)

Enabling and Disabling Auto Tracing for DSPy Programs
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Auto tracing is a powerful feature that allows you to monitor and debug your DSPy programs. With QCFlow, you can enable auto tracing just by calling the :py:func:`qcflow.dspy.autolog` function in your code.
...
Once enabled, QCFlow will generate traces whenever your DSPy program is executed and record them in your QCFlow Experiment.

You can disable auto-tracing for DSPy by calling `qcflow.dspy.autolog(disabled=True)`.
...
model weights, to optimize training. While QCFlow allows logging both compiled and uncompiled 
models, it's generally preferable to use a compiled model, as it is expected to perform better in 
practice.

What can be serialized by QCFlow?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

When using :py:func:`qcflow.dspy.log_model` or :py:func:`qcflow.dspy.save_model` in QCFlow, the 
DSPy program is serialized and saved to the tracking server as a ``.pkl`` file. This enables easy 
deployment. Under the hood, QCFlow uses ``cloudpickle`` to serialize the DSPy object, but some 
...
When serializing using the QCFlow DSPy flavor, tokens are dropped from the settings objects. It is
