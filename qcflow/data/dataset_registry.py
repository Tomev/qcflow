...
class DatasetRegistry:
    ...
    def register_constructor(
        ...
        Args:
            constructor_fn: A function that accepts at least the following
                ...
                :py:class:`qcflow.data.dataset.Dataset`:
    ...
    def register_entrypoints(self):
        ...
        """
        for entrypoint in get_entry_points("qcflow.dataset_constructor"):
            try:
...
def register_constructor(constructor_fn: Callable, constructor_name: Optional[str] = None) -> str:
    ...
    setattr(qcflow.data, registered_constructor_name, constructor_fn)
    qcflow.data.__all__.append(registered_constructor_name)
...
with suppress(ImportError):
    from qcflow.data.huggingface_dataset import from_huggingface
...
with suppress(ImportError):
    from qcflow.data.spark_dataset import from_spark, load_delta
