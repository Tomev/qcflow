...
def _validate_port_mapped_to_hostname(uri_parse):
    # This check is to catch an qcflow-artifacts uri that has a port designated but no
...
def _validate_uri_scheme(parsed_uri):
    ...
    if parsed_uri.scheme not in allowable_schemes:
        raise MlflowException(
            ...
            "1. Set the QCFLOW_TRACKING_URI environment variable to the desired tracking URI. "
            "`export QCFLOW_TRACKING_URI=http://localhost:5000`\n"
            "2. Set the tracking URI programmatically by calling `qcflow.set_tracking_uri`. "
