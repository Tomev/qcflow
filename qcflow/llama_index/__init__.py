...
def _validate_engine_type(engine_type: str):
    from qcflow.llama_index.pyfunc_wrapper import SUPPORTED_ENGINES

    if engine_type not in SUPPORTED_ENGINES:
        raise ValueError(
            f"Currently qcflow only supports the following engine types: "
...
def save_model(
    ...
    .. attention::
        ...
        Please refer to the `Models From Code Guide <https://www.qcflow.org/docs/latest/model/models-from-code.html>`_
        for more information.

    .. note::

        When logging a model, QCFlow will automatically save the state of the ``Settings``
    ...
    Args:
        ...
        model_config: The model configuration to apply when loading the model back with
            ``qcflow.pyfunc.load_model()``. It will be applied in a different way depending on the
    ...
    from qcflow.llama_index.serialize_objects import serialize_settings
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if isinstance(model_or_code_path, str):
            ...
            raise MlflowException.invalid_parameter_value(
                ...
                "https://www.qcflow.org/docs/latest/model/models-from-code.html",
            )
    ...
    pyfunc.add_to_model(
        qcflow_model,
        loader_module="qcflow.llama_index",
...
def log_model(
    ...
    **kwargs,
    ...
    """
    Log a LlamaIndex model as an QCFlow artifact for the current run.
    ...
    Args:
        ...
        model_config: The model configuration to apply when loading the model back with
            ...
            .. code-block:: python
                ...
                # When loading back, QCFlow will call ``index.as_chat_engine(top_k=10)``
                engine = qcflow.pyfunc.load_model(model_info.model_uri)

            For other model types saved with the `Model-from-Code <https://www.qcflow.org/docs/latest/model/models-from-code.html>`
            method, the config will be accessed via the :py:class`~qcflow.models.ModelConfig`
            ...
            .. code-block:: python
                ...
                model_config = qcflow.models.ModelConfig()
...
def load_model(model_uri, dst_path=None):
    ...
    """
    from qcflow.llama_index.serialize_objects import deserialize_settings
...
def autolog(
    ...
    """
    Enables (or disables) and configures autologging from LlamaIndex to QCFlow. Currently, QCFlow
    ...
    Args:
        ...
        silent: If ``True``, suppress all event logs and warnings from QCFlow during LlamaIndex
