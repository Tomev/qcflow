"""
The ``qcflow.spacy`` module provides an API for logging and loading spaCy models.
...
def save_model(
    ...
    if any(
        ...
        _logger.warning(
            ...
            "can be loaded back via `qcflow.spacy.load_model`, but cannot be loaded back using "
            "pyfunc APIs like `qcflow.pyfunc.load_model` or via the `qcflow models` CLI commands. "
            "QCFlow will only generate the pyfunc flavor for spacy models containing a pipeline "
...
def log_model(
    ...
    **kwargs,
    ...
    """Log a spaCy model as an QCFlow artifact for the current run.
...
def _load_pyfunc(path):
    ...
    Args:
        path: Local filesystem path to the QCFlow Model with the ``spacy`` flavor.
    """
...
def load_model(model_uri, dst_path=None):
    ...
    # Flavor configurations for models saved in QCFlow version <= 0.8.0 may not contain a
