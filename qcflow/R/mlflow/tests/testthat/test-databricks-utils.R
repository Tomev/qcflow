...
test_that("qcflow creates databricks client when scheme is databricks", {
  with_mock(.env = "qcflow", get_databricks_config = function(profile) {
    ...
    with_mock(.env = "qcflow", qcflow_rest = function(..., client) {
      ...
      client1 <- qcflow:::qcflow_client("databricks")
      ...
      client2 <- qcflow:::qcflow_client("databricks://dbprofile")
...
test_that("qcflow reads databricks config from correct sources", {
  with_mock(.env = "qcflow", get_databricks_config_for_profile = function(profile) list(
...
test_that("qcflow can read .databrickscfg files", {
   ...
   tryCatch(
    {
      ...
      write(profiles, file = config_file,
       ...
       profile1 <- qcflow:::get_databricks_config_for_profile("PROFILE1")
       ...
       expect_true(qcflow:::databricks_config_is_valid(profile1))

       profile2 <- qcflow:::get_databricks_config_for_profile("DONALD")
       ...
       expect_true(qcflow:::databricks_config_is_valid(profile2))

       profile3 <- qcflow:::get_databricks_config_for_profile("BROKEN1")
       ...
       expect_false(qcflow:::databricks_config_is_valid(profile3))

       profile4 <- qcflow:::get_databricks_config_for_profile("BROKEN2")
       ...
       expect_true(profile1$insecure == "False")
       expect_false(qcflow:::databricks_config_is_valid(profile4))
...
test_that("qcflow can read databricks env congfig", {
  env <- list(
  ...
  with_envvar( env, {
      envprofile <- qcflow:::get_databricks_config_from_env()
      ...
      expect_true(qcflow:::databricks_config_is_valid(envprofile))

      extracted_env <- qcflow:::databricks_config_as_env(envprofile)
...
test_that("databricks get run context fetches expected info for notebook environment", {
  ...
  expected_tags = list("5", "/path/to/notebook", "https://databricks-url.com",
                       QCFLOW_SOURCE_TYPE$NOTEBOOK, "/path/to/notebook")
  names(expected_tags) <- c(
    QCFLOW_DATABRICKS_TAGS$QCFLOW_DATABRICKS_NOTEBOOK_ID,
    QCFLOW_DATABRICKS_TAGS$QCFLOW_DATABRICKS_NOTEBOOK_PATH,
    QCFLOW_DATABRICKS_TAGS$QCFLOW_DATABRICKS_WEBAPP_URL,
    QCFLOW_TAGS$QCFLOW_SOURCE_TYPE,
    QCFLOW_TAGS$QCFLOW_SOURCE_NAME
  )

  run_with_mock_notebook_and_job_info(function() {
    context <- qcflow:::qcflow_get_run_context.qcflow_databricks_client(
      client = qcflow:::qcflow_client("databricks"),
      experiment_id = "52"
    )
...
test_that("databricks get run context fetches expected info for job environment", {
  ...
  expected_tags = list("10", "2", "notebook", "https://databricks-url.com",
                       QCFLOW_SOURCE_TYPE$JOB, "jobs/10/run/2")
  names(expected_tags) <- c(
    QCFLOW_DATABRICKS_TAGS$QCFLOW_DATABRICKS_JOB_ID,
    QCFLOW_DATABRICKS_TAGS$QCFLOW_DATABRICKS_JOB_RUN_ID,
    QCFLOW_DATABRICKS_TAGS$QCFLOW_DATABRICKS_JOB_TYPE,
    QCFLOW_TAGS$QCFLOW_SOURCE_TYPE,
    QCFLOW_TAGS$QCFLOW_SOURCE_NAME
  )
  ...
  run_with_mock_notebook_and_job_info(function() {
    context <- qcflow:::qcflow_get_run_context.qcflow_databricks_client(
      qcflow:::qcflow_client("databricks"),
      experiment_id = "52"
    )
    expect_true(all(expected_tags %in% context$tags))
    expect_true(context$experiment_id == "52")
