...
#' @export
qcflow_save_model.H2OModel <- function(model,
                                       path,
                                       model_spec = list(),
                                       conda_env = NULL,
                                       ...) {
  ...
  pip_deps <- list("qcflow", paste0("h2o==", as.character(utils::packageVersion("h2o"))))
...
#' @importFrom rlang %||%
#' @export
qcflow_load_flavor.qcflow_flavor_h2o <- function(flavor, model_path) {
  assert_pkg_installed("h2o")
...
#' @export
qcflow_predict.H2OModel <- function(model, data, ...) {
  assert_pkg_installed("h2o")
