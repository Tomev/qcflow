"""
The ``qcflow.fastai`` module provides an API for logging and loading fast.ai models. This module
...
def save_model(
    ...
    **kwargs,
    ...
    .. code-block:: python
        ...
        # Start QCFlow session and save model to current working directory
        with qcflow.start_run():
            model.fit(epochs, learning_rate)
            qcflow.fastai.save_model(model, "model")
...
def log_model(
    ...
    **kwargs,
    ...
    """Log a fastai model as an QCFlow artifact for the current run.
    ...
    .. code-block:: python
        ...
        def main(epochs=5, learning_rate=0.01):
            ...
            # Start QCFlow session and log model
...
def _load_pyfunc(path):
    ...
    Args:
        path: Local filesystem path to the QCFlow Model with the ``fastai`` flavor.
...
def autolog(
    ...
    """
    Enable automatic logging from Fastai to QCFlow.
    ...
    QCFlow will also log the parameters of the
    ...
    Args:
        log_models: If ``True``, trained models are logged as QCFlow model artifacts. If ``False``,
            trained models are not logged.
        log_datasets: If ``True``, dataset information is logged to QCFlow Tracking. If ``False``,
        ...
        disable_for_unsupported_versions: If ``True``, disable autologging for versions of fastai
            ...
            incompatible.
        silent: If ``True``, suppress all event logs and warnings from QCFlow during Fastai
    ...
    .. code-block:: python
        ...
        # https://github.com/qcflow/qcflow/tree/master/examples/fastai
    ...
    .. figure:: ../_static/images/fastai_autolog.png
        ...
        Fastai autologged QCFlow entities
    """
    ...
    def _log_model_info(learner):
        ...
        qcflow.log_param("input_size", inp_sz)
        qcflow.log_param("num_layers", len(infos))
    ...
    def _run_and_log_function(self, original, args, kwargs, unlogged_params, is_fine_tune=False):
        ...
        qcflow_cbs = [cb for cb in self.cbs if cb.name == "___qcflow_fastai"]
        fit_in_fine_tune = (
            original.__name__ == "fit" and len(qcflow_cbs) > 0 and qcflow_cbs[0].is_fine_tune
        )
        ...
        with batch_metrics_logger(run_id) as metrics_logger:
            if not fit_in_fine_tune:
                ...
                qcflowFastaiCallback = getFastaiCallback(
                    metrics_logger=metrics_logger, is_fine_tune=is_fine_tune
                )

                # Add the new callback
                self.add_cb(qcflowFastaiCallback)
