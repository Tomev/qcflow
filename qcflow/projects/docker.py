...
from qcflow.projects.utils import QCFLOW_DOCKER_WORKDIR_PATH
from qcflow.utils import file_utils, process
from qcflow.utils.databricks_utils import get_databricks_env_vars
from qcflow.utils.file_utils import _handle_readonly_on_windows
from qcflow.utils.git_utils import get_git_commit
from qcflow.utils.qcflow_tags import QCFLOW_DOCKER_IMAGE_ID, QCFLOW_DOCKER_IMAGE_URI

_logger = logging.getLogger(__name__)

_GENERATED_DOCKERFILE_NAME = "Dockerfile.qcflow-autogenerated"
_QCFLOW_DOCKER_TRACKING_DIR_PATH = "/qcflow/tmp/mlruns"
_PROJECT_TAR_ARCHIVE_NAME = "qcflow-project-docker-build-context"
...
def build_docker_image(work_dir, repository_uri, base_image, run_id, build_image, docker_auth):
    ...
    if not build_image:
        ...
        dockerfile = (
            f"FROM {base_image}\n COPY {_PROJECT_TAR_ARCHIVE_NAME}/ {QCFLOW_DOCKER_WORKDIR_PATH}\n"
            f" WORKDIR {QCFLOW_DOCKER_WORKDIR_PATH}\n"
        )
    ...
    tracking.MlflowClient().set_tag(run_id, QCFLOW_DOCKER_IMAGE_URI, image_uri)
    tracking.MlflowClient().set_tag(run_id, QCFLOW_DOCKER_IMAGE_ID, image.id)
    return image
...
def _create_docker_build_ctx(work_dir, dockerfile_contents):
    ...
    try:
        dst_path = os.path.join(directory, "qcflow-project-contents")
...
def get_docker_tracking_cmd_and_envs(tracking_uri):
    ...
    if local_path is not None:
        cmds = ["-v", f"{local_path}:{_QCFLOW_DOCKER_TRACKING_DIR_PATH}"]
        env_vars[QCFLOW_TRACKING_URI.name] = container_tracking_uri
...
def _get_local_uri_or_none(uri):
    ...
    if not parsed_uri.netloc and parsed_uri.scheme in ("", "file", "sqlite"):
        ...
        if parsed_uri.scheme == "sqlite":
            uri = file_utils.path_to_local_sqlite_uri(_QCFLOW_DOCKER_TRACKING_DIR_PATH)
        else:
            uri = file_utils.path_to_local_file_uri(_QCFLOW_DOCKER_TRACKING_DIR_PATH)
        return path, uri
    else:
        return None, None
