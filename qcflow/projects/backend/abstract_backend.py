...
class AbstractBackend:
    """
    Abstract plugin class defining the interface needed to execute QCFlow projects. You can define
    subclasses of ``AbstractBackend`` and expose them as third-party plugins to enable running
    QCFlow projects against custom execution backends (e.g. to run projects against your team's
    in-house cluster or job scheduler). See `QCFlow Plugins <../../plugins.html>`_ for more
    information.
    """
    ...
    def run(
        ...
        Args:
            project_uri: URI of the project to execute, e.g. a local filesystem path
                or a Git repository URI like https://github.com/qcflow/qcflow-example
            ...
            backend_config: A dictionary, or a path to a JSON file (must end in '.json'), which
                ...
                documented at https://www.qcflow.org/docs/latest/projects.html.
        ...
        Returns:
            A :py:class:`qcflow.projects.SubmittedRun`. This function is expected to run
