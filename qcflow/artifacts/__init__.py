"""
APIs for interacting with artifacts in QCFlow
"""
...
def download_artifacts(
    ...
    Args:
        ...
        run_id: ID of the QCFlow Run containing the artifacts. Exactly one of ``run_id`` or
            ``artifact_uri`` must be specified.
        artifact_path: (For use with ``run_id``) If specified, a path relative to the QCFlow
...
def load_text(artifact_uri: str) -> str:
    ...
    .. code-block:: python
        ...
        with qcflow.start_run() as run:
            ...
            qcflow.log_text("This is a sentence", "file.txt")
            file_content = qcflow.artifacts.load_text(artifact_uri + "/file.txt")
            print(file_content)
...
def load_dict(artifact_uri: str) -> dict:
    ...
    .. code-block:: python
      ...
      with qcflow.start_run() as run:
          artifact_uri = run.info.artifact_uri
          qcflow.log_dict({"qcflow-version": "0.28", "n_cores": "10"}, "config.json")
          config_json = qcflow.artifacts.load_dict(artifact_uri + "/config.json")
          print(config_json)

    .. code-block:: text
      :caption: Output

      {'qcflow-version': '0.28', 'n_cores': '10'}
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        local_artifact = download_artifacts(artifact_uri, dst_path=tmpdir)
        with open(local_artifact) as local_artifact_fd:
...
def load_image(artifact_uri: str):
    ...
    .. code-block:: python
        ...
        with qcflow.start_run() as run:
            ...
            image = qcflow.artifacts.load_image(artifact_uri + "/image.png")
            print(image)
